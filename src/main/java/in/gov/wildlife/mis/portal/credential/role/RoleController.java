...
//    }
    ...
    @PostMapping("/insertRole")
    public ResponseEntity<?> insertRole(@Valid @RequestBody List<RoleDto> roleMDto) {

        if (roleMDto.size() > 1) {
            // Batch save using saveAll() method
            List<Role_m> roles = roleMDto.stream().map((roleDto) -> modelMapper.map(roleDto, Role_m.class)).collect(Collectors.toList());
            List<Role_m> savedRoles = roleMRepository.saveAll(roles);
            return new ResponseEntity<>(savedRoles, HttpStatus.CREATED);
        } else if (roleMDto.size() == 1) {
            // Single save using save() method
            Role_m role = modelMapper.map(roleMDto.get(0), Role_m.class);
            Role_m savedRole = roleMRepository.save(role);
